...
int rand(int r)			//random a number in the r range
{
	srand(time(NULL));
	return rand() % r;
}

int botMove(int size, char(&board)[100][100],char sign)			//check for the almost winner
{
	for (int i = 0; i < size; i++)
	{
		for (int j = 0;j < size; j++)
		{
            ...
            else if(board[i][j]==sign && board[i+1][j-1]==sign && board[i+2][j-2]==' ')
            {
                board[i+2][j-2]='O';
                return 1;
            }
             else if(board[i][j]==sign && board[i+1][j-1]==sign && board[i-1][j+1]==' ')
            {
                board[i-1][j+1]='O';
                return 1;
            }
...
int botRandom(int size, char(&board)[100][100])
{
	...
	for (int i = 0; i < size; i++)
	{
		for (int j = 0; j < size; j++)
		{
			if (board[i][j] == ' ')				//check for the empty board
			{
				aRnd[k].first = i;				//pass the empty board coordinate to an array
				aRnd[k].second = j;
				k++;
			}
		}
	...
	int r = rand(k);
	board[aRnd[r].first][aRnd[r].second] = 'O';		//random element in the array
	return 0;
}
