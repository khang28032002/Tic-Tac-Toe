// Tic - Tac - Toe v2.0
//It still have many failed test cases and a lot to improve
...
int main()
{
    ...
    else
    {
        do
        {
            ...
            system("CLS");
            if (botMove(size, board,cSign) == -1)           //if the bot is about to win, make that move
            {
                if (botMove(size,board,pSign) == -1)        //if the player is about to win, stop the player
                {
                    botRandom(size, board);                 //if not the above option then random the move
                    displayBoard(board, size);
                }
                else
                {
